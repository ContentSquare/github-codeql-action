...
import {
  ...
  getCacheSaveKey,
  getCompatibleCodeQlVersionsForOverlayBaseDatabases,
} from "./caching";
...
test.serial(
  "getCompatibleCodeQlVersionsForOverlayBaseDatabases returns unique versions sorted latest first",
  async (t) => {
    const logger = getRunnerLogger(true);
    const config = createTestConfig({
    ...
    const result = await getCompatibleCodeQlVersionsForOverlayBaseDatabases(
      config,
      logger,
    );
...
test.serial(
  "getCompatibleCodeQlVersionsForOverlayBaseDatabases returns empty list when no caches exist",
  async (t) => {
    const logger = getRunnerLogger(true);
    const config = createTestConfig({ languages: ["python"] });
...
test.serial(
  "getCompatibleCodeQlVersionsForOverlayBaseDatabases returns empty list when cache keys are unparseable",
  async (t) => {
    const logger = getRunnerLogger(true);
    const config = createTestConfig({ languages: ["python"] });
...
test.serial(
  "getCompatibleCodeQlVersionsForOverlayBaseDatabases returns the single version when only one cache exists",
  async (t) => {
    const logger = getRunnerLogger(true);
    const config = createTestConfig({ languages: ["cpp"] });
...
test.serial(
  "getCompatibleCodeQlVersionsForOverlayBaseDatabases resolves language aliases",
  async (t) => {
    const logger = getRunnerLogger(true);
    // The alias `c++` should be resolved to "cpp" and match cache entries keyed with "cpp"
    const config = createTestConfig({ languages: ["c++"] });
